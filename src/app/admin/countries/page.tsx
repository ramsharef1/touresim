...
import { adminListCountries } from '../lib'
        ...
        <div className="rounded-xl border border-zinc-800 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-zinc-900 text-zinc-400">
              <tr>
                <th className="px-4 py-3 text-left">ISO2</th>
            ...
            <tbody>
              {countries.map((c) => (
                <tr key={c.id} className="border-t border-zinc-800 hover:bg-zinc-900/50">
                  <td className="px-4 py-3 font-mono font-bold">{c.iso2}</td>
                  ...
                  <td className="px-4 py-3">
                    <a href={`/admin/countries/${c.id}`} className="text-amber-400 hover:underline">Edit →</a>
                  </td>
                </tr>
              ))}
            </tbody>
